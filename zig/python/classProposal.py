...
class ProposalDb:
    ...
    def put(self, proposal):
        try:
            if (
                self.get_by_title_and_type(proposal.title, proposal.presentationType)
                is not None
            ):
                return {
                    "status": "ng",
    ...
    def get_by_title_and_type(self, title, presentationType):
        response = self.table.scan(
            FilterExpression="title = :title and presentationType = :presentationType",
    ...
    def update(self, proposal):
        self.table.update_item(
            Key={"title": proposal.title},
            UpdateExpression="SET presentationType = :presentationType, abstract = :abstract, estimated_time = :estimated_time, seminar = :seminar, speakers = :speakers",
            ExpressionAttributeValues={
                ":presentationType": proposal.presentationType,
    ...
    def query(self, seminar):
        response = self.table.query(
            KeyConditionExpression="seminar = :seminar",
            ExpressionAttributeValues={":seminar": seminar},
        )
        if "Items" not in response:
            return []
        return [Proposal(item) for item in response["Items"]]
    ...
    def query_by_speaker(self, speaker):
        response = self.table.scan(
            FilterExpression="contains(speakers, :speaker)",
            ExpressionAttributeValues={":speaker": speaker},
        )
        if "Items" not in response:
            return []
        return [Proposal(item) for item in response["Items"]]
    ...
    def query_by_seminar(self, seminar):
        response = self.table.scan(
            FilterExpression="seminar = :seminar",
            ExpressionAttributeValues={":seminar": seminar},
        )
        if "Items" not in response:
            return []
    ...
    def query_by_speaker_and_seminar(self, speaker, seminar):
        response = self.table.scan(
            FilterExpression="contains(speakers, :speaker) and seminar = :seminar",
            ExpressionAttributeValues={":speaker": speaker, ":seminar": seminar},
        )
        if "Items" not in response:
            return []
        return [Proposal(item) for item in response["Items"]]
    ...
    def query_by_speaker_and_seminar_and_type(self, speaker, seminar, presentationType):
        response = self.table.scan(
            FilterExpression="contains(speakers, :speaker) and seminar = :seminar and presentationType = :presentationType",
            ExpressionAttributeValues={
                ":speaker": speaker,
                ":seminar": seminar,
                ":presentationType": presentationType,
    ...
    def query_by_speaker_and_type(self, speaker, presentationType):
        response = self.table.scan(
            FilterExpression="contains(speakers, :speaker) and presentationType = :presentationType",
            ExpressionAttributeValues={
                ":speaker": speaker,
                ":presentationType": presentationType,
            },
